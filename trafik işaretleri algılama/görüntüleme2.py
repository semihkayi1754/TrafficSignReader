...
#VİDEO KAMERAYI AYARLAYIN
cap= cv2.VideoCapture(0)
cap.set(3,frameWidth)
cap.set(4,frameHeight)
cap.set(10,brightness)

pickle_in=open("model_trained.pt","rb")
model=pickle.load(pickle_in)

def preprocess_img(imgBGR, erode_dilate=True):  # görüntüdeki işaretleri tespit etmek için ön işleme.
...
#def new_func(model, crop_img):
#    return model.predict_classes(crop_img)
    
def new_func(model, crop_img, threshold=0.5):
    # Modelin tahminini yap
    predictions = model.predict(crop_img)
    
    # Tahmin edilen sınıf indeksini bul
    predicted_class_index = np.argmax(predictions, axis=-1)
    
    # En yüksek olasılık değerini al
    max_probability = np.max(predictions, axis=-1)
    
    # Eğer bir eşik değeri belirtilmişse ve en yüksek olasılık bu eşiği geçmiyorsa sınıfı -1 olarak ayarla
    if threshold is not None and max_probability < threshold:
...
if __name__ == "__main__":
    ...
    while (1):
        ...
        rects = contour_detect(img_bin, min_area=min_area)   # x,y,h ve w'yi elde edin.
        img_bbx = img.copy()
        for rect in rects:
            ...
            # rect[2] genişliktir ve rect[3] yüksekliktir
            if rect[2] > 100 and rect[3] > 100:             #yalnızca yüksekliği ve genişliği >100 olan işaretleri tespit edin
            ...
            crop_img = crop_img.reshape(1, 32, 32, 1)       # (1,32,32) yeniden şekillendirildikten sonra (1,32,32,1) olur
            predictions = model.predict(crop_img)           # tahmin etmek
            classIndex = new_func(model, crop_img)
            probabilityValue = np.amax(predictions)
            if probabilityValue > threshold:
                # çıkış ekranına sınıf adını yaz
                cv2.putText(img_bbx, str(classIndex) + " " + str(getCalssName(classIndex)), (rect[0], rect[1] - 10),
                            font, 0.75, (0, 0, 255), 2, cv2.LINE_AA)
                # çıkış ekranına olasılık değerini yaz
                cv2.putText(img_bbx, str(round(probabilityValue * 100, 2)) + "%", (rect[0], rect[1] - 40), font, 0.75,
                            (0, 0, 255), 2, cv2.LINE_AA)

        cv2.imshow("detect result", img_bbx)
        if cv2.waitKey(1) & 0xFF == ord('q'):           # çıkmak için q
            break
cap.release()
